...
$basequery= safe_input($_GET['product']);
$basequery= str_replace('-', ' ', $basequery);
$html_scr= endpoint_search($basequery, $pages);
...
$nextpage= $pages+1;
$nexttoken= string2key($nextpage);
$QUERY_NAME= str_replace("/", "%2F", $basequery);
...
$pagestitle= "Shop ".$QUERY_NAME." catalogs - Buy ".$QUERY_NAME." for unbeatable low prices on ".$_SERVER['SERVER_NAME'].$pagesname;
$metadesc= "Shop ".$QUERY_NAME." catalogs - Online Shopping ".$QUERY_NAME.", Browse Through Our Directory of ".$QUERY_NAME.$pagesname.", ".joincategory($morecats).", Worldwide Shipping!";
...
<meta name="robots" content="NOINDEX,FOLLOW,NOARCHIVE,NOODP,NOYDIR,NOTRANSLATE"/>
<style>
	...
	<div class="fluid">
    	<div class="header">
			<form action="/search" method="GET" role="form" target="_top">
				...
				<input class="m-submit" type="submit" value="Cari" />
			</form>
            <div class="clear"></div>
        </div>
    </div>
...
<h1 class="title" itemprop="name">Shop <?php echo $QUERY_NAME;?> catalogs - Buy <?php echo $QUERY_NAME;?> for unbeatable low prices</h1>	
         <div class="itemLists"> 
<?php
...
<div class="pagination">
		...
		if($pages >= 3){
			$prevpage= ($pages-1);
			$prevtoken= string2key($prevpage);
			echo '<li><a href="/search?product='.urlencode($QUERY_NAME).'&page='.$prevpage.'&token='.$prevtoken.'">PREV PAGE</a></li>'; 
		}
		if($html_scr['nextpage']){
			echo '<li><a href="/search?product='.urlencode($QUERY_NAME).'&page='.$nextpage.'&token='.$nexttoken.'">NEXT PAGE</a></li>'; 
		}
...
<div class="m-footer">
    ...
    <div class="footer">
    	<div class="fluid">
        	<div class="foot-desc">
        	Search results are displayed by <a href="https://www.google.com/webmasters/sitemaps/ping?sitemap=http://<?php echo $_SERVER['SERVER_NAME'];?>" target="_blank"><?php echo $_SERVER['SERVER_NAME'];?></a> is as a reference. Suitability price, details, specifications, pictures and other information is the responsibility of the seller. <br>By using <?php echo $_SERVER['SERVER_NAME'];?> services, you agree to comply with this regulation.
            </div>
